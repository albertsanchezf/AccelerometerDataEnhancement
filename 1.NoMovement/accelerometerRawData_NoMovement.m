...
% Plot Filtered Data
figure;
suptitle('Accelerometer induced error HPF (0.35 Hz)')
subplot(3,1,1)
plot(ts,xHP1)
ylabel('x'); xlabel('t(s)');
subplot(3,1,2)
...
%% Plot Filtered Data
figure;
suptitle('Accelerometer induced error LPF1 (0-1.9 Hz)')
subplot(3,1,1)
plot(ts,xLP1)
ylabel('x'); xlabel('t(s)');
subplot(3,1,2)
...
lp2 = designfilt('lowpassfir','FilterOrder',70,'CutoffFrequency',fc2, ...
       'DesignMethod','window','Window',{@kaiser,3},'SampleRate',Fs);
...
ylabel('z');xlabel('t(s)');
hold off;
legend('Raw','HPF','LPF(0-3Hz)','LPF(4.9Hz)','BPF(0.5-3Hz)');
