...
% - Z: Matrix(M,N) with the N signals from Acc_Z and the M samples
    ...
    if find(idx == 1)
        x(:,1) = Acc_X{Incident_Type}(1:lengths(1));
        y(:,1) = Acc_Y{Incident_Type}(1:lengths(1));
        z(:,1) = Acc_Z{Incident_Type}(1:lengths(1));
    else
        x(:,1) = resample(Acc_X{Incident_Type}(1:lengths(1)),maxSize,lengths(1));
        y(:,1) = resample(Acc_Y{Incident_Type}(1:lengths(1)),maxSize,lengths(1));
        z(:,1) = resample(Acc_Z{Incident_Type}(1:lengths(1)),maxSize,lengths(1));
    end
    ...
    if length(lengths) > 1
        if find(idx == length(lengths))
            x(:,length(lengths)) = Acc_X{Incident_Type}(inx(end)+1:inx(end)+lengths(end));
            y(:,length(lengths)) = Acc_Y{Incident_Type}(inx(end)+1:inx(end)+lengths(end));
            z(:,length(lengths)) = Acc_Z{Incident_Type}(inx(end)+1:inx(end)+lengths(end));
        else
            x(:,length(lengths)) = resample(Acc_X{Incident_Type}(inx(end)+1:inx(end)+lengths(end)),maxSize,lengths(end));
            y(:,length(lengths)) = resample(Acc_Y{Incident_Type}(inx(end)+1:inx(end)+lengths(end)),maxSize,lengths(end));
            z(:,length(lengths)) = resample(Acc_Z{Incident_Type}(inx(end)+1:inx(end)+lengths(end)),maxSize,lengths(end));
        end
    end
